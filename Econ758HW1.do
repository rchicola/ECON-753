...
esttab WithChildren1993  WithChildrenPost1993 WithoutChildren1993 WithoutChildrenPost1993 using Table3.rtf, rtf cell(mean) unstack nonumber title("Labor Force Participation Rate of Women") mtitle("Pre1993" "Post1993") replace


bysort year: egen WithChildren1993 = mean(work) if child==1 & post1993==0
bysort year: egen YearmeanChildren = mean(work) if child==1 
...
esttab Unconditional Conditional using Table4.rtf, rtf se star(* 0.10 ** 0.05 *** 0.01) title("Children Versus NoChildren EITC Expansion") mtitle(Unconditional Conditional) scalars(N r2 pval F) replace	
esttab Unconditional Conditional using Table4.tex, tex se star(* 0.10 ** 0.05 *** 0.01) title("Children Versus NoChildren EITC Expansion") mtitle(Unconditional Conditional) scalars(N r2 pval F) replace	
